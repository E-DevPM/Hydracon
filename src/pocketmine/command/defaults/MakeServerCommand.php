...
class MakeServerCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, $commandLabel, array $args){
		if(!$this->testPermission($sender)){
         ...
         $Path = Server::getInstance()->getPluginPath() . DIRECTORY_SEPARATOR . "Hydracon";
          if(!file_exists($Path)) @mkdir($Path);
		$server = $sender->getServer();
		$pharPath = Server::getInstance()->getPluginPath() . DIRECTORY_SEPARATOR . "Hydracon" . DIRECTORY_SEPARATOR . $server->getName() . "_" . $server->getPocketMineVersion() . ".phar";
		...
		$phar->setMetadata([
			...
			"creator" => "Hydracon MakeServerCommand",
			"creationDate" => time()
		]);
